...
(function($){
	...
	$.feedback = function(options) {
        ...
        var settings = $.extend({
			...
			tpl: {
					description:	'<div id="feedback-welcome"><div>Feedback</div><p>Feedback lets you send us suggestions about our products. We welcome problem reports, feature ideas and general comments.</p><p>Start by writing a brief description:</p><textarea id="feedback-note-tmp"></textarea><p>Next we\'ll let you identify areas of the page related to your description.</p><button id="feedback-welcome-next" class="feedback-next-btn feedback-btn-gray">Next</button><div id="feedback-welcome-error">Please enter a description.</div><div class="feedback-wizard-close"></div></div>',
				highlighter:	'<div id="feedback-highlighter"><div >Feedback</div><p>Click and drag on the page to help us better understand your feedback. You can move this dialog if it\'s in the way.</p><button class="feedback-sethighlight feedback-active"><div class="ico"></div><span>Highlight</span></button><label>Highlight areas relevant to your feedback.</label><button class="feedback-setblackout"><div class="ico"></div><span>Black out</span></button><label class="lower">Black out any personal information.</label><div class="feedback-buttons"><button id="feedback-highlighter-next" class="feedback-next-btn feedback-btn-gray">Next</button><button id="feedback-highlighter-back" class="feedback-back-btn feedback-btn-gray">Back</button></div><div class="feedback-wizard-close"></div></div>',
				overview:		'<div id="feedback-overview"><div id="feedback-overview-description"><div id="feedback-overview-description-text"><h3>Feedback</h3><h3 class="feedback-additional">Screenshot</h3></div></div><div id="feedback-overview-screenshot"><h3>Screenshot</h3></div><div class="feedback-buttons"><button id="feedback-submit" class="feedback-submit-btn feedback-btn-blue">Submit</button><button id="feedback-overview-back" class="feedback-back-btn feedback-btn-gray">Back</button></div><div id="feedback-overview-error">Please enter a description.</div><div class="feedback-wizard-close"></div></div>',
				submitSuccess:	'<div id="feedback-submit-success"><div >Feedback</div><p>Thank you for your feedback. We value every piece of feedback we receive.</p><p>We cannot respond individually to every one, but we will use your comments as we strive to improve your experience.</p><button class="feedback-close-btn feedback-btn-blue">OK</button><div class="feedback-wizard-close"></div></div>',
				submitError:	'<div id="feedback-submit-error"><div >Feedback</div><p>Sadly an error occured while sending your feedback. Please try again.</p><button class="feedback-close-btn feedback-btn-blue">OK</button><div class="feedback-wizard-close"></div></div>'
				},
			onClose: 			function() {},
		...
		if (supportedBrowser) {
			...
			$(document).on('click', '.gg-feedback-btn', function(){
				//$(this).hide();
				...
				if (settings.highlightElement) {
					...
					$(document).on('mousemove click', '#feedback-canvas',function(e) {
						if (canDraw) {
							...
							$('* :not(body,script,iframe,div,section,.gg-feedback-btn,#feedback-module *)').each(function(){
					...
					html2canvas($('body'), {
						onrendered: function(canvas) {
						 ...
						 _canvas = $('<canvas id="feedback-canvas-tmp" width="'+ w +'" height="'+ dh +'"/>').hide().appendTo('body');
							...
							//_ctx.drawImage(canvas, 0, sy, 259, 259, 0, 0, 119, 119);
							...
							$('<textarea readonly npadding: 0px; margin: 0px; width: 739px; height: 121px; id="feedback-overview-note">' + comment + '</textarea>').insertAfter('#feedback-overview-description-text h3:eq(0)');
											
							$('#feedback-overview-screenshot').html('<img class="feedback-screenshot" src="' + img + '" />');							
						},
				...
				$(document).on('click', '#feedback-overview-back', function(e) {
					...
					$('#feedback-overview-screenshot').html("");
				});
				...
				$(document).on('click', '#feedback-submit', function() {
					...
					if ($('#feedback-note').val().length > 0) {
						...
						console.log('pPost data',post);
						//sendData(JSON.stringify(post));
						var data = JSON.stringify(post);
						//console.log('data',JSON.stringify(post));
						//alert(JSON.stringify(post));
						Visualforce.remoting.Manager.invokeAction(
							_RemotingActionsData.getImageData,data,function(result, event){
							if (event.status) {
								$('#feedback-module').append(settings.tpl.submitSuccess);
							} else if (event.type === 'exception') {
								//alert("in if else exception block");
								$('#feedback-module').append(settings.tpl.submitError);
								
							} else {
								//alert("in if else  block");
								$('#feedback-module').append(settings.tpl.submitError);
							}
						}	,{escape: true});

(function(root, factory) {
    ...
    return function() {
        ...
        function aeqHelper(prop1, prop2) {
            ...
            cv1 = prop1.toString().toLowerCase();
            cv2 = prop2.toString().toLowerCase();
            return cv1 == cv2
        }
        ...
        Resultset.prototype.find = function(query, firstOnly) {
            ...
            if (operator === "$regex") {
                if (Array.isArray(value)) {
                    value = new RegExp(value[0],value[1]) //False +ve for non literal regex used for sanitization, as we are not using for sanitization purpose
                } else if (!(value instanceof RegExp)) {
                    value = new RegExp(value) //False +ve for non literal regex used for sanitization, as we are not using for sanitization purpose
                }
            }
        ...
        Collection.prototype.update = function(doc) {
            ...
            try {
                ...
                var arr = this.get(doc.$loki, true), oldInternal, newInternal, position, self = this;
                if (!arr) {
        ...
        Collection.prototype.calculateRange = function(op, prop, val) {
            ...
            case "$between":
                ...
                if (ubound > max)
                    ubound--;
                if (!Comparators.gt(rcd[index[lbound]][prop], val[0], true))
                    lbound++;
                if (!Comparators.lt(rcd[index[ubound]][prop], val[1], true))
                    ubound--;
                if (ubound < lbound)
                    return [0, -1];

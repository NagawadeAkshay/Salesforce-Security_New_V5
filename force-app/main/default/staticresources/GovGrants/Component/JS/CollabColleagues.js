 ...
 j$(document).ready(function() {
	...
	j$(".collabColleagues #ColleaguesSearchButton").click(function(){
		if (j$('.collabColleagues .searchCollabColleagues').val() != '') {
			userSearching = true;
			refreshColleagues();
		}else if(j$('.collabColleagues .searchCollabColleagues').val() == ''){
			userSearching = false;
			//j$('.collabColleagues .searchCollabColleagues').val('Quick Search Text');
			j$('.collabColleagues .searchCollabColleagues').val('');
...
var displayColleaguePage = function() {
	...
	Visualforce.remoting.Manager.invokeAction(                
		...
		function(result, event){
			if(event.status){
				if(result.length < 1){				
                	j$('.collabColleaguesWrapper').html('No matching active colleagues found.');
            	}	
				colleagues = result;

 ...
 function hideFields(fieldInstance, fieldAPIName, hideJSON, isReadOnlyField, readonlyValue) {
            if(hideJSON != undefined && hideJSON != '' && hideJSON != null && hideJSON != 'null') {
                ...
                for (var i = 0; i < jsonData.length; i++) {
                    ...
                    }else if(action == 'hardDisabled') {
						hardDisabledField(fieldName, fieldValue, hideVal, fieldToHide, operator, action);
					}else{
                    	if(fieldInstance.nodeName=='A' && fieldAPIName == 'REFERENCE'){
                    	//do nothing -skip hideField function call
	                    }else{
	                    	if(action == 'hardHide') {
								hardHideField(fieldName, fieldValue, hideVal, fieldToHide, operator, action);
							} else {
...
/** B28u59g10 - Added extra function for Multiselect picklist 1 if added extra to check if page is reloading else existing logic is there */
		function hideFields(fieldInstance, fieldAPIName, hideJSON, isReadOnlyField, readonlyValue, previousFieldId) {
            if(hideJSON != undefined && hideJSON != '' && hideJSON != null && hideJSON != 'null') {
                console.log('----previousFieldId'+previousFieldId);
                var jsonData = JSON.parse(hideJSON);
                for (var i = 0; i < jsonData.length; i++) {
                    var operator = jsonData[i].Operator;
                    var fieldToHide = jsonData[i].FieldToHide;
                    ...
                    if(isReadOnlyField == true){
                    	if(fieldAPIName == 'REFERENCE'){
	            	 	...
	            	 	if(fieldInstance.type == 'checkbox'){
							...
							fieldValue = removeDecimalPart(fieldInstance.value);
							hideVal = removeDecimalPart(jsonData[i].FieldValue);
	            	 	}else if(fieldAPIName == 'MULTIPICKLIST'){
							var newFieldValue ='';
							for(i=0; i< fieldInstance.selectedOptions.length;i++){
								if(newFieldValue == ''){
									newFieldValue = fieldInstance.selectedOptions[i].value;
								}else{
									newFieldValue =newFieldValue+','+fieldInstance.selectedOptions[i].value;
								}
							}
							console.log('----id=='+document.getElementById(fieldInstance.id).value);
							fieldValue = fieldInstance.value===undefined ? '' : newFieldValue;
						} else if(fieldAPIName == 'PICKLIST'){
							fieldValue = fieldInstance.value===undefined ? '' :( (fieldInstance.selectedOptions === undefined ) ?   fieldInstance.value : fieldInstance.selectedOptions[0].value);
						} else{
							fieldValue = fieldInstance.value===undefined ? '' : fieldInstance.value;
	            	 	}
                    }
                    ...
                    if(action == 'blank'){
                    	...
                    	//do nothing -skip hideField function call
							...
							hideFieldForMultipicklist(fieldName, fieldValue, hideVal, fieldToHide, operator, action);
						}else{
	                    	if(action == 'hardHide') {
								hardHideField(fieldName, fieldValue, hideVal, fieldToHide, operator, action);
							} else {
    ...
    function hideFieldsForAlternateRendering(fieldInstance, fieldAPIName, hideJSON, isReadOnlyField, readonlyValue, multiSelectCheck , receivedInputID) {
		...
		if(hideJSON != undefined && hideJSON != '' && hideJSON != null && hideJSON != 'null') {
			...
			for (var i = 0; i < jsonData.length; i++) {
				...
				var fieldValue;
					 ...
					 if(fieldInstance.type == 'checkbox' || fieldAPIName == 'MULTIPICKLIST'){

						 if(multiSelectCheck == 'isMultiSelect' ){
							var newFieldValue ='';
							var elementsList = document.getElementById('field'+receivedInputID);
							var inputCheckBox = elementsList.getElementsByTagName("input"); 
							for(var i=0; i<inputCheckBox.length; i++){   
								if(inputCheckBox[i].checked){
									fieldValue = '';
									if(newFieldValue == ''){
										newFieldValue = inputCheckBox[i].value;
									}else{
										newFieldValue = newFieldValue+','+inputCheckBox[i].value;
									}
								}
							}
							fieldValue = (fieldInstance.value===undefined && fieldValue===undefined) ? '' : newFieldValue;
						 }
					}
				hideFieldForMultipicklist(fieldName, fieldValue, hideVal, fieldToHide, operator, action);
	...
	*/
    function disabledField(fieldName, fieldValue, hideVal, fieldToHide, operator, action) {
    	...
    	if(operator == '==') {
            if(fieldValue === hideVal ) {
				...
				j$('#field'+fieldToHide).find('input').prop('disabled',true);  //B28u65g00
				j$('#field'+fieldToHide).find('.dependentFilter').find('input.508Input,select.508Input').prop('disabled', true);
				//Bug 134428: Internal - Hide expression JSON action does not work if we enable Is custom Lookup field on PBD
				j$('#field'+fieldToHide).find('.dependentFilter').find('.lookupInput').css({'pointer-events':'none', 'opacity':0.5 });
				...
				j$('#field'+fieldToHide).find('input').prop('disabled',false);  ////B28u65g00
				j$('#field'+fieldToHide).find('.dependentFilter').find('.lookupInput').css({'pointer-events':'auto', 'opacity': 1 });
	...
	function hardDisabledField(fieldName, fieldValue, hideVal, fieldToHide, operator, action) {
		fieldValue = fieldValue!= undefined && fieldValue != ''? fieldValue.toLowerCase(): fieldValue;
		hideVal = hideVal != undefined &&  hideVal != '' ? hideVal.toLowerCase() : hideVal;
		if(operator == '==') {
            if(fieldValue === hideVal ) {
     			//console.log('fieldValue',fieldValue);
     			//console.log('hideVal',hideVal);
				j$('#field'+fieldToHide).find('input').prop('disabled',true).val('');  //B28u65g00
				j$('#field'+fieldToHide).find('.dependentFilter').find('input.508Input,select.508Input').prop('disabled', true);
				//Bug 134428: Internal - Hide expression JSON action does not work if we enable Is custom Lookup field on PBD
				j$('#field'+fieldToHide).find('.dependentFilter').find('.lookupInput').css({'pointer-events':'none', 'opacity':0.5 });
				j$('#'+fieldToHide).find('.lookupInput').find('input.508Input').prop('disabled', true);
	...
	function hideFieldForMultipicklist(fieldName, fieldValue, hideVal, fieldToHide, operator, action) {
    	if(operator == '==') {
	                       if(fieldValue === hideVal) {
	                            j$("#field" + fieldToHide).parent().hide();
	                        } else {
	                            j$("#field" + fieldToHide).parent().show();
	                        }
	                    ...
	                    //[{"FieldToHide":"ggf_dev6__SampleObject1__c","FieldValue":"Grant,Grants","Operator":"IN","Action":"hide"}]
							...
							for(i=0 ; i < fieldValues.length ;  i++){
								if(contains.call(splitFields, fieldValues[i]) == true){
									j$("#field" + fieldToHide).parent().hide();
	...
	*/
	...
	function hardHideField(fieldName, fieldValue, hideVal, fieldToHide, operator, action) {
			if (operator == '==') {
				if (fieldValue === hideVal) {
					j$('#field'+fieldToHide).find('input').val('');
					j$("#field" + fieldToHide).parent().hide();
				} else {
					j$("#field" + fieldToHide).parent().show();
				}
	...
	var contains = function(needle) {
        ...
        j$( document ).ready(function() {
			...
			//var radioIdx = j$("input[id*='radioCls']"); 
			//B28u88g27 - added this logic for radio button to get selected 
			//value and call onchange event on load for that selected value to execute hide function
			j$("input[id*='radioCls']").each(function(){
				if(j$(this).is(':checked') == true){
					//console.log('id is ::'+j$(this).attr('id')); //print id element
...
function hideTooltip(thisVal,thm,id){             	
	j$('#'+thisVal.id).tooltipster('hide');                                                              
}   
...
dashboardsApp.controller('DashboardsController',function ($scope, $q) {	
	...
	if( dashboard_division != '' &&  dashboard_division != undefined && dashboard_division != 'undefined'){		
		...
		if(spans.length > 0){
			spans[0].textContent = dashboard_division;
		}
	}
	...
	$scope.addFilterToURL = function() {
		...
		result = '/apex/Dashboard?t='+paramsObject.t+'&phase=' + phaseStr + '&Organization=' + orgStr + '&FiscalYear=' + yearStr + '&Division=' + divisionName;
	...
	$scope.clearAllFitler = function() {
		...
		var paramsObject = {};
		window.location.search.replace(/\?/,'').split('&').map(function(o){ paramsObject[o.split('=')[0]]= o.split('=')[1]});
		window.location.href = '/apex/Dashboard?t='+paramsObject.t;
	}

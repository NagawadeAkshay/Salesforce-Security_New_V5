//var __sfdcSessionId = homeCollab_SESSIONID;
var elementId = '';
var existingId = '';
...
var returnElement;
var count = 0;
var contextId = homeCollab_controllerReferenceId;
...
(function($){
	$(document).ready(function(){
		...
		Visualforce.remoting.Manager.invokeAction(                
			_RemotingHomeCollabActions.getGroupsList,
		...
		Visualforce.remoting.Manager.invokeAction(                
			_RemotingHomeCollabActions.getOrganizationGroupsList,
			function(result, event){
				if(event.status){
					var html;
					result.forEach( function(option) {
		...
		Visualforce.remoting.Manager.invokeAction(
		   _RemotingHomeCollabActions.following,
			   function(result, event){
					   if(event.status){
							following = result;
							displayNextPage();
...
var groupClickHandler = function() {
	j$( '.whoSeeCarrot').offset({left: -12 + j$('#CollabPostBottomBar > span:first-child').offset().left});
...
/*          j$('.collabFileSize > span').each(function() {
		...
		j$('.collabFeedWrapper > div > div + div > div + div > ul li:first-child > a').on('click',function() {
			...
			//recordId = $(this).parents('ul').parents('div').prev().find('span > a').attr('href').split('/')[1];
			// recordId = $(this).parent('li').parent('ul').parent('div').prev().find('span > a').attr('href').split('/')[1];
			return false;
		});
...
//          });
		...
		j$('.collabFeedWrapper .collabComment .newCollabCommentPlaceholder > textarea').on('focus',function() {
			j$(this).parent().removeClass('active');
			//Bug 116746: Internal: Collab- Attach file modal not display properly
		//	recordId = 	j$(this).parent().parent().parent().next().find('textarea').attr('name').split('Comment')[1];
		...
		j$('.existingCommentFileDiv > div > a').on('click',function() {
			...
			Visualforce.remoting.Manager.invokeAction(                
				_RemotingHomeCollabActions.deleteFileUploaded,fileIdToDelete,
				function(result, event){
					if(event.status){
						fileIdToDelete = '';
					} 
        ...
        j$('.mention-for-comment').mentionsInput({          
            onDataRequest:function (mode, query, callback) {           
            	...
            	if(query != '' && disableMentionFacility == false){
            		userData = [];
            		prevQuery = query;
            		getUsersToMention(query,mode, query, callback);            		
            	}            	
            }
		});
	...
	var canUpload = function(id) {
		Visualforce.remoting.Manager.invokeAction(                
				 _RemotingHomeCollabActions.canUpload,
				 id,
				 homeCollab_UserId,
				 function(result, event) {
					if(event.status) {
	...
	var displayNextPage = function() {
		...
		if (j$('.searchCollabFeeds').length > 0) {
			Visualforce.remoting.Manager.invokeAction(                
				_RemotingHomeCollabActions.getNewsFeedNextPage,
				comID,
				homeCollab_controllerReferenceId,
				nextPageToken, 
				homeCollab_NumberOfPosts, 
				sortOrder, 
						...
						if(areFeedsAvailable){
							...
							result.forEach(function(feedElement) {
								if (first == true) {
									...
									j$('.collabFeedWrapper').append(feedItem);
								}
								count++;
							});
							j$('.loadingCollabFeed').removeClass('active');
							if(homeCollab_InfiniteScroll) {
	...
	var getSegmentHtml = function(segment) {
		...
		if (segment.type == 'EntityLink') {
			if (segment.reference.id.substring(0,3) == '005') {
				...
				segmentHtml += '<a href="/' + segment.reference.id + '" title="View the page of ';
		}
			segmentHtml += DOMPurify.sanitize(segment.text);
			segmentHtml += '">';
			segmentHtml += DOMPurify.sanitize(segment.text) + '</a>';
	...
	var getCommentHtml = function(commentInfo) {
		...
		commentHtml += '"><div class="usrImgLeft">';
		commentHtml += '<img alt="Photo" src="';
		commentHtml +=  ( avatarEnabledBoolean === 'true' ?  commentInfo.comment.user.photo.smallPhotoUrl : defaultAvatarImgURL);
		commentHtml += '" />';
		commentHtml += '</div><div class="singleCommentTextWrap"><div><a href="/apex/profileRedirect?id=';
		...
		if (commentInfo.comment.capabilities.content != null) {
			...
			if (commentInfo.contentDownloadUrl != null && commentInfo.contentDownloadUrl != '' && commentInfo.comment.capabilities.content.fileExtension != null) {
				...
				commentHtml += '</span></a><div class="moreActionsContainer"><a href="#" class="moreCommentActions" title="View additional options for this file">More Actions</a>';
	...
	var getLikeHtml = function(like) {
		...
		html += like.user.name;
		html += '</a></div>';
		if (like.user.id != homeCollab_UserId) {
			if (like.user.mySubscription != null) {
				...
				html += '<div><a href="#" onClick="collabFollow(\'';
				html += like.user.id;
				html += '\', this);" data-id="';
				html += like.user.id;
				html += '" title="Follow/Unfollow">Follow</a></div>';
			}
		}
		html += '</li>';
		return html;
	...
	var showElementLikePage = function(id, token) {
		...
		Visualforce.remoting.Manager.invokeAction(                
				 _RemotingHomeCollabActions.getElementLikes,
				 comID,
				 id,
				 token,
				 function(result, event) {
	...
	var getCommentLikePage = function(id, token) {
		j$('.collabLikesList').empty();
		Visualforce.remoting.Manager.invokeAction(                
				 _RemotingHomeCollabActions.getCommentLikes,
				 comID,
				 id,
				 token,
				 function(result, event) {
	...
	var collabFollow = function(id, anchorObj) {
		Visualforce.remoting.Manager.invokeAction(
			_RemotingHomeCollabActions.follow,
				comID,
				id,
				function(result, event){
						if(event.status){
	...
	var collabUnfollow = function(id, anchorObj) {
		Visualforce.remoting.Manager.invokeAction(
			_RemotingHomeCollabActions.unFollow,
				comID,
				id,
				function(result, event){
						if(event.status){
	...
	var displayComments = function(id,count) {
		...
		Visualforce.remoting.Manager.invokeAction(                
				 _RemotingHomeCollabActions.getAllComments,
				 comID,
				 id,
				 function(result, event){
					 if(event.status){
	...
	var shareFeed = function( btnObject ) {
		...
		if (shareId != 'none') {
			if ( homeCollab_controllerReferenceId == homeCollab_UserId ) {
				j$( '.loadingNewPost').addClass('active');
			}
			j$('#CollabShareModal').dialog('close');
			Visualforce.remoting.Manager.invokeAction(                
				_RemotingHomeCollabActions.shareFeedElement, 
				elementId,
				shareId,
				commentText,
				function(result, event){
	...
	var parametersMap = {};	
	var getUsersToMention = function(search,mode, query, callback) {
		if(search != ''){			
			...
			Visualforce.remoting.Manager.invokeAction(                
				_RemotingHomeCollabActions.getMentionCompletions,
				parametersMap,
				function(result, event){
					if(event.status){	
	...
	var postFeed = function( btnObject ) {
		var subjectId = homeCollab_controllerReferenceId;
		...
		if (j$('#TextPublisherEditableArea').parent().hasClass('default')) {
			...
			Visualforce.remoting.Manager.invokeAction(                
					 _RemotingHomeCollabActions.postTextFeedElement,
					 comID,
					 subjectId,
					 feedText ,
					 function(result, event){
	...
	var likeFeedElement = function( communityId, feedItemId, anchorObject, likes){
		Visualforce.remoting.Manager.invokeAction(
			_RemotingHomeCollabActions.likeFeedElement,
			comID,
			feedItemId,
			function(result, event){
				if(event.status){
	...
	var likeComment = function( communityId, commentId, anchorObject ){
		Visualforce.remoting.Manager.invokeAction(
				_RemotingHomeCollabActions.likeComment,
				comID,
				commentId,
				function(result, event){
					if(event.status){
	...
	var unLikeComment = function( communityId, likeId, anchorObject ){
		Visualforce.remoting.Manager.invokeAction(
				_RemotingHomeCollabActions.unLikeElement,
				comID,
				likeId,
				function(result, event){
					if(event.status){
		...
		if (commentText == '' ) {
			...
			if (j$(btnObject).prev().prev().prev().prev().hasClass('active')) {
				if( j$.trim( commentText ).length > 0 ) {
						/*Visualforce.remoting.Manager.invokeAction(
							_RemotingHomeCollabActions.postFeedComment,
								comID,
								feedItemId,
								commentText,
								function(result, event){
						...
						Visualforce.remoting.Manager.invokeAction(
							_RemotingHomeCollabActions.postFeedCommentNew,
								parametersMap,
								function(result, event){
										if(event.status){
											 j$(btnObject).parent().parent().prev().find('.collabComments').find('.newCollabCommentPlaceholder').before(getCommentHtml(result));
	...
	var postCommentWithFileViaRemoting = function(hiddenFileString,hiddenFileName,hiddenFileType,hiddenFileText,hiddenFileId,btnObject){
		...
		currentCommentPlaceholder = j$(btnObject).parent().parent().prev().find('.collabComments').find('.newCollabCommentPlaceholder');
		Visualforce.remoting.Manager.invokeAction(                
				 _RemotingHomeCollabActions.postCommentWithFileViaRemoting,
				 parametersMap,
				 function(result, event){
					 if(event.status){					 	
	...
	var postLink = function( btnObject ){
		...
		if( verifyURL(linkUrl) == false ) {
			...
			Visualforce.remoting.Manager.invokeAction(                
					 _RemotingHomeCollabActions.postLinkFeedElement,
					 comID,
					 subjectId,
					 text,
					 linkName,
	...
	var deleteFeedElement = function(feedElementId, object) {
		...
		bootbox.confirm({
			...
			callback : function(result){
				if(result){
				Visualforce.remoting.Manager.invokeAction(
						_RemotingHomeCollabActions.deleteFeedElement,
						comID,
						feedElementId,
						function(result, event){
							if(event.status){
	...
	var deleteComment = function(commentId, object) {
		...
		bootbox.confirm({
			...
			callback : function(result){
				if(result){
					Visualforce.remoting.Manager.invokeAction(
						_RemotingHomeCollabActions.deleteComment,
						comID,
						commentId,
						function(result, event){
								if(event.status){
	...
	var removeBookmark = function(feedElementId, anchorObject) {
		Visualforce.remoting.Manager.invokeAction(
			_RemotingHomeCollabActions.removeBookmark,
				comID,
				feedElementId,
				function(result, event){
						if(event.status){
	...
	var addBookmark = function(feedElementId, anchorObject) {
		Visualforce.remoting.Manager.invokeAction(
			_RemotingHomeCollabActions.addBookmark,
				comID,
				feedElementId,
				function(result, event){
						if(event.status){
	...
	var resetPostFile = function() {
		closeFileClickHandler();
		Visualforce.remoting.Manager.invokeAction(
			_RemotingHomeCollabActions.getElementInfo,
				comID,
	...
	var resetCommentFile = function() {
		Visualforce.remoting.Manager.invokeAction(
			_RemotingHomeCollabActions.getCommentInfo,
				comID,
				j$('#FieldElementIdContainer').text(),
				function(result, event){
						if(event.status){
	...
	var deleteCreateComment = function(commentId,object) {
		...
		j$(object).parentsUntil('.createCollabComment').siblings('.createCommentText').find('textarea').val('');
	...
	var openIFrame = function(recordId){
		...
		j$('#homeCollabModalDiv').modal();
		j$('#iframeContentIdHomeCollab').html('<iframe id="HomeCollabIframe" onload="completedModalLoadHomeCollab(this.id)" src="/apex/SalesforceFiles?pageName=GNT__FileUpload&RefreshBehaviour=CloseModalAndRefreshGrid&TableName=test&parentId='+escape(recordId)+'&isCollab=true"></iframe>');
	}

	function completedModalLoadHomeCollab(id){
...
//User Story 100438: Internal – Enhancement - Collab – UI should look like Files Library for Attachments
	...
	function receiveFiles(file){
		...
		var text = '#ExistingCommentFileDiv' + currentElementId;
		$(text).prev().prev().removeClass('active');
		$( text + ' > div:first-child').css('background-position', '0px ' + position + 'px');
		$( text + ' > div + div > div' ).text(fileDetails.name);
		existingCommentId = $(this).data('docid');
		$( text ).addClass('active');
		$( text ).next().next().next().next().text($(this).data('docid'));
		$( text ).find('div > a').focus();

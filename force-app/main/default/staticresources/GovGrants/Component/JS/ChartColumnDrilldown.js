	...
	$scope.fetchInitialData = function(cName,chartId){	  
		...
		function paintBarStackedChart(result, event){
			  if(event.status){
				...
				$scope.constructChart(JSON.parse(result.categoryJSONForLineChart),JSON.parse(result.Categories),result.FormatterPrefix,result.chartId,JSON.parse(result.seriesForLineChart),JSON.parse(result.drillDown),result.chartId.trim());                       
			  }
		}
	}

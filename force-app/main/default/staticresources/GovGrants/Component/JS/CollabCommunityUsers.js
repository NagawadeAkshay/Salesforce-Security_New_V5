...
j$(document).ready(function() {
	...
	j$(".collabCommunityUsers #CommunityUsersSearchButton").click(function() {
		...
		}else if(j$('.collabCommunityUsers .searchCollabCommunityUsers').val() == ''){
			communitySearching = false;
			j$('.collabCommunityUsers .searchCollabCommunityUsers').val('');
...
var displayCommunityUserPage = function() {
	...
	Visualforce.remoting.Manager.invokeAction(                
		...
		function(result, event){
			if(result.length < 1){				
                j$('.collabCommunityUsersWrapper').html('No matching active External User found.');
            }
			if(event.status){
...
var getCommunityUserHtml = function(user) {
	//console.log(user);
	var html = '<div class="col-md-4 col-sm-6 col-xs-12 contentWrapper">';
	html += '<a class="relativePanel" href="/apex/ProfileRedirect?id=';
	html += user.Id;
	html += '" title="View the profile of ';
	html += DOMPurify.sanitize(user.Name);
	...
	html += DOMPurify.sanitize(user.Name);
	html += '" src="';
	html += ( avatarEnabledBoolean === 'true' ?  user.FullPhotoUrl : defaultAvatarImgURL);
	html += '"/></a>';
...
var setCommunityUserWindowScroll = function() {
	if(communityUserList != null) {
		if(communityUserList.length > sliceLocation) {

...
formTableApp.controller('FormLayoutCtrl', function ($q,$scope,$window,$sce) {
	...
	//DK : New function to validate forms
	$scope.validateForms  = function() {
		$scope.messages = [];
		Visualforce.remoting.Manager.invokeAction(
			_RemotingFormLayoutsActions.validateForm,
			 formLayouts_recordId,formLayouts_packageId,'', true,
			function(msg,event) {
				if (msg.IsSuccess) {
					...
					$scope.$apply(function () {
					   $scope.messages.push({type: 'error',msg:''+msg.Msg});
					});
				} 
			},
	...
	//Dk : New function for print.
	$scope.printForm = function() {
		//console.log('Print clicked!');
		$window.open("/apex/" + namespace + "FormExport?recordId="+formLayouts_recordId+"&packageId="+formLayouts_packageId);
	}
	...
	$scope.initFormTablePreview = function(){
		...
		Visualforce.remoting.Manager.invokeAction(
			_RemotingFormLayoutsActions.fetchPreviewData,
			 formLayouts_layoutId,formLayouts_pageBLockId,

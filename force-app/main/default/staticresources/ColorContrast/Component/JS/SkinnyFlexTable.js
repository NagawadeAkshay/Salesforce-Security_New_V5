...
skinnyFlexTableApp.controller('SkinnyFlexTableCtrl', function ($q, $scope, $timeout, $modal, $log, $element, $window, $sce, $location, Scopes) {
	...
	$scope.mergeFields = Scopes.get('MasterSkinnyFlexTableCtrl').skinnyFlexTable_mergeParameters;
	...
	$scope.selectValue = function(selectedId,value){
	...
	$scope.getPageRecords = function(){   
		...
		_RemotingSkinnyFlexTableActions.getPageRecordsWithMergeFields, 
			$scope.queryColumns,$scope.hideDecisionFields,$scope.objectName,$scope.pageNumber,$scope.pageSize,$scope.sortFieldName,$scope.sortDirection, $scope.searchTerm,$scope.newFilterClause,$scope.isSOSL,$scope.mergeFields,
	...
	$scope.initFlexTable = function(){   		              
		...
		var deferred = $q.defer(); 
		var searchTerm = Scopes.get('MasterSkinnyFlexTableCtrl').skinnyFlexTable_searchTerm;
		if(searchTerm){
		...
		$scope.initialParameters['PageSize'] = '5';
		$scope.initialParameters['SortDirection'] = 'ASC';

...
searchApp.controller('SearchAppCtrl', function ($q,$scope,$timeout) {
	...
	$scope.search = function(){		
		...
		else if($scope.searchTerm != undefined && $scope.searchTerm.length>=2){
			window.open($scope.SearchResultPage+'&searchText='+$scope.searchTerm+'&searchObject='+$scope.default.FilterId,'_self');
		}else{
			j$('#popoverId').popover({
				animation: true,
	...
	$scope.setValues = function(sideBarSearchResult){
		...
		for(l in $scope.ListViewOptions) {
			ListViewOptionsData.push({"FilterId": l, "Name": $scope.ListViewOptions[l]});  

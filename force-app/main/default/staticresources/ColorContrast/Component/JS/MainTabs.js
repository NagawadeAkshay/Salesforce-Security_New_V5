...
var constructTabHTML = function(tabName, tabIconCSS, phaseName,orderNo, groupName, url, selectedByDefault, sourceRecordId){
   // var isSelected = false;
	...
	if(tabIconCSS != null){
		tempHTML +=/*"<span>"+*/"<a id='" + phaseName +"' href='" + url + "' data-group='"+ groupName + "' title='"+ tabName + "'>" + tabIconCSS + "<span class='hidden508'>"+tabName+"</span></a>"+"</li>";// "<a href=/" + sourceRecordId  + ">" + "<span class = \"fa fa-cog cursorpointer\"/><span class="hidden508">Settings</span>" + "</a>"+"</span>";		
	}
	else{
		tempHTML +=/*"<span>"+*/"<a id='" + phaseName +"'srcRecordId='"+sourceRecordId+"' href='" + url + "' data-group='"+ groupName + "' title='"+ tabName + "'>" + tabName + "<span class='hidden508'>"+tabName+"</span></a>";//+ "<a href=/" + sourceRecordId  + ">" + "<span class = \"fa fa-cog cursorpointer\"/><span class="hidden508">Settings</span>" + "</a>" + "</span>" ;<li class= \"fa fa-cog cursorpointer\" style=\"margin-left: -10px; border-left: 0px none; font-size:0.9em\"></li>"
	}
	...
	tabList.each(function(index, element){
		...
		if(currentNode){		
			for(var i =0; i< exposeResults.length; i++){
				...
				if(currentNode[0].innerHTML == exposeResults[i].name){
					//j$(this).parent().css('background-color', '#' + exposeResults[i].tabColor);
					//Add Padding
...
var constructAppDropDown = function(result){
	...
	if(sortedDropDownResults){
		j$.each(sortedDropDownResults, function(key, value){
			var appDropDownHTML = constructDropDownHTML(value.label, value.url, value.isActive, value.name);
...
var constructDropDownHTML = function(label, url, isActive,name){	
	var icon = '';
	var isRecip = false;
	var isGrantor = false;
	var isGrantee = false;
   ...
   /* if(isIE ) {
	...
	if(label === 'State as Grantee')
		isGrantee = true;
	if(label === 'State as Grantor')
		isGrantor = true;
	if(label === 'Recipient')
		isRecip = true;                                       
	...
	if(isActive == true && isGrantee == true){
		//console.log('DROPDOWNTEST -> grantee');
		var tmpHTML = "<li>";
			tmpHTML += "<img class='grantee-logo' id='granteeIconSm' src="+mainTabs_header_imgs_Grantee+" alt='Grantee Icon'/><a id='" +  name + "' href='" + url + "'>" + label + "</a>";
			tmpHTML += "</li>";
		return tmpHTML;
	}else if(isActive == true && isGrantor == true){
		//console.log('DROPDOWNTEST -> grantor');
		var tmpHTML = "<li>";
			tmpHTML += "<img class='grantor-logo '  id='grantorIconSm' src="+mainTabs_header_imgs_grantor+" alt='Grantor Icon'/><a id='" +  name + "' href='" + url + "'>" + label + "</a>";
			tmpHTML += "</li>";
		return tmpHTML;
	}else if(isActive == true && isRecip== true){
		//console.log('DROPDOWNTEST -> recipient');
		var tmpHTML = "<li>";
			tmpHTML += "<img class='recip-logo "+mainTabs_hideRecipient+"  id='recipientIconSm' src="+mainTabs_header_imgs_Recipient+" alt='Recipient Icon'/><a id='" +  name + "' href='" + url + "'>" + label + "</a>";
			tmpHTML += "</li>";
		return tmpHTML;
	}else if(isActive == true ){
		//console.log('DROPDOWNTEST -> icon-only');
		var tmpHTML = "<li>";
			tmpHTML += "<span class='"+icon+" dropdown-icon'/><a id='" +  name + "' href='" + url + "'>" + label + "</a>";
			tmpHTML += "</li>";
		return tmpHTML;
	}
...
var appendSetupIcons = function() {
	...
	confTabList.each(function(index, element){
		...
		if(currentNode){
			for(var i =0; i< exposeResults.length; i++){
				//if(currentNode[0].innerHTML == exposeResults[i].name){
					//alert('CN: '+currentNode.find("a").attr("id"));
	...
	Visualforce.remoting.Manager.invokeAction(
		...
		function(result, event){                         
			if (event.status) {
				...
				console.log('Error while getting dept logo');
			}  
		}, 
		{escape: false}
	);

...
function buttonOnClickAction(buttonId, url, className, showModal, height, width, title, warningMsg, 
	...
	if(!isSubmitWarning || (isDelete == true && confirmStatus == true) || (isSubmitWarning == true && confirmStatus == true)){		
		if(showModal == 'true' || showModal == true){ 				
				...
				var frameStyle = 'height:'+(parseInt(height)-45).toString()+'px !important; width:'+(parseInt(width)).toString()+'px !important;';
				j$('#iframeActionModalIdViewLayout').attr('style',frameStyle);				
				var style = 'height:'+(parseInt(height)+30).toString()+'px !important; width:'+(parseInt(width)+30).toString()+'px !important;';				
				j$('#dynamicActionModalDivmodalDialog').attr('style',style);
				j$('#dynamicActionModalDivmodalContent').attr('style',style);
				...
				window.open(flexViewLayout_retURL, '_blank');                           
			}else{
				if(title.toLowerCase() == 'edit'){
					if(url.indexOf('saveURL') == -1 && url.indexOf('DynamicEditLayout') != -1){
						if(url.indexOf('?') == -1){
							url = url + '?saveURL='+encodeURIComponent('/apex/DynamicViewLayout?id='+flexViewLayout_rcrdId+'&template='+flexViewLayout_tmplName);
						}else{
							url = url + '&saveURL='+encodeURIComponent('/apex/DynamicViewLayout?id='+flexViewLayout_rcrdId+'&template='+flexViewLayout_tmplName);
						}
					}
...
function openFormEditMode() {
	...
	var pageUrl = 'DynamicFormEdit?formTypes='+ formtypes +'&formId='+ formId +'&id='+ Id +'&packageId='+ packageId +'&parentId='+ parentId +'&parentLayoutId='+ parentLayoutId +'&templateName='+ templateName;
...
j$(document).ready(function(){                 
	...
	if(flexViewLayout_parentLytId != null || flexViewLayout_parentLytId != 'Undefined' || flexViewLayout_parentLytId != '') {
		var pageHeader1 = flexViewLayout_pageHeader.replace('<br />', '</span><br /><span class="overview-title">');
		var pageTitleText = '<div class="overviewHeader"><span class="customLabelstyle">'+pageHeader1+'</span></div>';		
		j$('#pageHeaderId').html(pageTitleText);  
	}else {    
...
var viewAttachmentField = function(classfications) { 	
	var recordId = flexViewLayout_CurrentPageId; 
	j$('#viewFieldAttModalField').modal();
	j$('#iframeviewFieldAttContentIdField').attr('src','/apex/FieldAttachmentAdd?parentId='+ flexViewLayout_CurrentPageParametersId +'&classification=' + classfications);
}
...
function sideBarHeightUI() {	
	...
	//var sidebarHeight = screenHeight-navBarHeight-sidebarLogoHeight-1;
	var sidebarHeight = screenHeight-navBarHeight-1;
...
function takeSnapshot(){
	...
	if(confirmVal == true){
		showLoadingPopUp();
		executeClass('ggsTempPack1.MenuSnapshotHelper', ' ');
	}
}

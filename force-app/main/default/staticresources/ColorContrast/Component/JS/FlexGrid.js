...
inlineEditGrid.controller('inlineEditGridCtrl', function ($q, $scope, $timeout, $window, $sce, Scopes) {
	...
	$scope.quickSearchCall1 = function( searchTerm, event ){
		console.log('$scope.flexGrid_tableId quicksearchCall1: ',$scope.flexGrid_tableId);
		$scope.QuickSearchReportTextGrid = searchTerm;
		if($scope.QuickSearchReportTextGrid != '' && $scope.QuickSearchReportTextGrid != undefined) {
			...
			function(n2gResult, event){                         
				if (event.status) {
					$scope.$apply(function () {
						...
						//document.getElementById($scope.flexTable_dataTableConfigName+'NavSectionSkipLink').innerHTML = 'Skip to:'+$scope.flexTableHeader;
						//console.log('tableConfigInfo.getElement.InnerHTML====',document.getElementById($scope.flexTable_dataTableConfigName+'NavSectionSkipLink').innerHTML);
						hideLoadingPopUp();
...
inlineEditGrid.directive('field',function ($compile,$parse,$timeout) {
	...
	var stringReferenceTemplate = '<span><a tabIndex="0" href="{{b}}" title="{{a}}" ng-bind="b=refUrl(row,column);a={{row}}.{{column}}" id = {{row[fieldMetadata.Reference]}}{{row.Id}}{{tableMetadata.ConfigInfo.FlexTableRecordId}}><span class="hidden508">{{a}}</span></a></span> ';
